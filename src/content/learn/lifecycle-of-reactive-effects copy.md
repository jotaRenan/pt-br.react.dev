...
- O significado de um array de dependências
- Como o React verifica se suas dependências estão corretas através de um linter
- O que fazer quando você discordar do linter
...
- Um componente se _monta_ quando é adicionado à tela.
- Um componente se _atualiza_ quando recebe novas props ou estado, geralmente em resposta a uma interação.
- Um componente se _desmonta_  quando é removido da tela.

**É uma boa maneira de se pensar sobre componentes, mas _não_ sobre Effects.** Ao invés disso, tente pensar em Effects de maneira independente do ciclo de vida do seu componente. Um Effect descreve como [sincronizar um sistema externo](/learn/synchronizing-with-effects) com o estado e props atuais. Conforme seu código muda, sincronização precisará acontecer mais ou menos frequentemente.
...
O corpo do seu Effect especifica como **começar a sincronizar-se:**
...
Intuitivamente, você pode pensar que o React **começaria a sincronizar-se** quando seu componente se monta e **pararia de sincronizar-se** quando seu componente se desmonta. Entretanto, esse não é o fim da história! Às vezes também pode ser necessário **começar e parar uma sincronização várias vezes** enquanto o componente permanece montado.

Vamos ver _porque_ isso é necessário, _quando_ isso acontece e _como_ você pode controlar esse comportamento.

<Note>

Alguns Effects nem retornam uma função de limpeza. [Vai ser mais comum você querer retorná-las do que não](/learn/synchronizing-with-effects#how-to-handle-the-effect-firing-twice-in-development)--mas, se você não o fizé-lo, o React vai se comportar como se você tivesse retornado uma função de limpeza vazia.

</Note>

### Porque uma sincronização pode precisar acontecer mais de uma vez {/*why-synchronization-may-need-to-happen-more-than-once*/}

Imagine que este componente `ChatRoom` recebe uma prop `roomId` escolhida por um usuário em um menu de seleção. Digamos que o usuário inicialmente seleciona `"general"` como `roomId`. Seu app exibe a sala `"general"` no chat:
...
Após a UI ser exibida, o React executará seu Effect para **começar a sincronização**. Ele se conecta à sala `"general"`:
...
Pouco depois, o usuário escolhe outra sala no menu de seleção (por exemplo, `"travel"`). Primeiro, o React atualizará a UI:
...
Pense no que acontecerá em seguida. O usuário verá que `"travel"` é a sala selecionada na UI. Porém, o Effect executado da última vez ainda está conectado à sala `"general"`. **A prop `roomId` mudou, então o que seu Effect fez aquela hora (conectar-se à sala `"general"`) não condiz mais com a UI.**

Nesse ponto, você quer que o React faça duas coisas:

1. Pare de sincronizar com a antiga `roomId` (desconectar-se da sala `"general"`)
2. Comece a sincronizar com a nova `roomId` (conectar-se à sala `"travel"`)

**Por sorte, você já ensinou ao react como fazer essas duas coisas!** O corpo do seu Effect especifica como iniciar a sincronização e a função de limpeza especifica como parar de sincronizar-se. Tudo que o React precisa fazer agora é invocá-las na ordem correta e com as props e estado corretos. Vejamos exatamente como isso funciona.

### Como o React ressincroniza seu Effect {/*how-react-re-synchronizes-your-effect*/}

Lembre-se que seu componente `ChatRoom` recebeu um novo valor para sua prop `roomId`. Ela valia `"general"` e agora vale `"travel"`. O React precisa ressincronizar seu Effect e reconectar você a uma sala diferente.

Para **parar de se sincronizar**, o React chamará a função de limpeza do seu Effect retornada após se conectar à sala `"general"`. Como `roomId` valia `"general"`, a função de limpeza se disconecta da sala `"general"`:
...
Em seguida, o React executará o Effect que você disponibilizou durante essa renderização. Dessa vez, `roomId` vale `"travel"`, então ele **começará a sincronizar-se** com a sala de chat `"travel"` (até que sua função de limpeza sejá eventualmente chamada, também):
...
Toda vez após seu componente se rerrenderizar com uma `roomId` diferente, seu effeito irá se ressincronizar. Digamos, por exemplo, que o usuário muda a `roomId` de `"travel"` para `"music"`. O React irá novamente parar de sincronizar seu Effect ao chamar a função de limpeza (desconectando você da sala `"travel"`). Em seguida, **começará a sincronizar-se** novamente através da execução do corpo do Effect com o novo valor da prop `roomId` (conectando você à sala `"music"`).

Por fim, quando o usuário for a uma tela diferente, `ChatRoom` é desmontada. Nesse momento, não haverá mais motivo para se manter conectado. O react irá **parar de sincronizar** seu Effect uma última vez e desconectar-se da sala de chat`"music"`.
...
1. Seu Effect se conectou à sala `"general"`
2. Seu Effect se desconectou da sala `"general"` e conectou-se à sala `"travel"`
3. Seu Effect se desconectou da sala `"travel"` e conectou-se à sala `"music"`
...
```js
  useEffect(() => {
    //  Seu Effect conectou-se à sala especificada pelo roomId...
    const connection = createConnection(serverUrl, roomId);
    connection.connect();
    return () => {
      // ...até se desconectar
...
A estrutura desse código pode te inspirar a enxergar o que aconteceu como uma sequência de intervalos de tempo que não se sobrepõem uns aos outros:

1. Seu Effect conectou-se à sala `"general"` (até se desconectar)
2. Seu Effect conectou-se à sala `"travel"` (até se desconectar)
3. Seu Effect conectou-se à sala `"music"` (até se desconectar)

Até então, você estava pensando a partir da perspectiva do componente. A partir dela, era tentador interpretar Effects como "callbacks" ou "eventos do ciclo de vida" disparados em momentos específico, como "depois da renderização" ou "antes da desmontagem". Esse modelo de pensamento gera complicações bem rápido, então é melhor ser evitado.

**Ao invés disso, sempre foque em um único ciclo de início/fim de cada vez. Não deveria importar se um componente está se montando, atualizando ou desmontando. Tudo que você precisa fazer é descrever como iniciar a sincronização e como pará-la. Se fizer isso bem, seu efeito aguentará ser iniciado ou interrompido quantas vezes forem necessárias.**

Isso talvez te lembre que você nem pensa se um componente está se montando ou desmontando quando escreve a lógica de renderização que cria JSX. Você descreve o que deveria estar na tela e o React [cuida do resto](/learn/reacting-to-input-with-state).

### Como o React verifica que seu Effect pode ressincronizar {/*how-react-verifies-that-your-effect-can-re-synchronize*/}

Aqui está um exemplo interativo e editável. Clique em "Open Chat" para montar o componente `ChatRoom`:
...
Note que, quando o componente se monta pela primeira vez, você vê três logs:

1. `✅ Conectando-se à sala "general" em https://localhost:1234...` *(apenas em modo de desenvolvimento)*
1. `❌ Desconectado da sala "general" em https://localhost:1234.` *(apenas em modo de desenvolvimento)*
...
**O React verifica que seu Effect pode se ressincronizar forçando-o a fazer isso imediatamente em desenvolvimento.** Isso pode lembrá-lo de abrir uma porta e fechá-la uma vez a mais para ver se a fechadura funciona. O React inicia e interrompe o seu Effect uma vez a mais em desenvolvimento para conferir [se você implementou sua função de limpeza direito.](/learn/synchronizing-with-effects#how-to-handle-the-effect-firing-twice-in-development)

Na prática, a principal razão para seu Effect ressincronizar é se algum dado usado por ele mudar. No sandbox acima, altere a sala selecionada. Note que, como `roomId` muda, seu Effect ressincroniza.

Entretanto, há alguns casos mais incomuns em que ressincronizar é necessário. Por exemplo, tente editar `serverUrl` no sandbox acima enquanto o chat está aberto. Note como o Effect ressincroniza em resposta às suas alterações no código. No futuro, o React pode adicionar mais funcionalidades que se baseiem em ressincronização.

### Como o React sabe que precisa ressincronizar o Effect {/*how-react-knows-that-it-needs-to-re-synchronize-the-effect*/}


Você pode estar pensando como o React sabe que seu Effect precisa ser ressincronizado após uma mudança em `roomId`. É porque *você disse ao React* que o código dele depende de `roomId` ao incluí-la na [lista de dependências:](/learn/synchronizing-with-effects#step-2-specify-the-effect-dependencies)

```js {1,3,8}
function ChatRoom({ roomId }) { // O valor da prop roomId pode mudar ao longo do tempo
  useEffect(() => {
    const connection = createConnection(serverUrl, roomId); // Esse Effect lê roomId
    connection.connect();
    return () => {
      connection.disconnect();
    };
  }, [roomId]); // Então você diz ao React que esse Effect precisa de roomId
  // ...
```
...
1. Você sabe que `roomId` é uma prop, o que quer dizer que seu valor pode mudar com o tempo.
Você sabe que seu Effect lê `roomId` (então a lógica dele depende do valor que pode mudar depois).
3. Por isso você especificiou ela como dependência do seu Effect (para que ele se ressincronize quando `roomId` mudar)..

Toda vez após seu componente se rerrenderizar, o React irá olhar para a lista de dependências que você passou. Se algum dos valores na lista for diferente do valor naquela mesma posição durante a renderização anterior, o React irá ressincronizar seu Effect.

Por exemplo, se você passou `["general"]` durante a renderização inicial, e depois passou `["travel"]` na renderização seguinte, o React irá comparar `"general"` com `"travel"`. Esses valores são diferentes (comparados com [`Object.is`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/is)), então o React irá ressincronizar seu Effect. Por outro lado, se seu componente rerrenderizar mas `roomId` não tiver mudado, seu Effect irá continuar conectado à mesma sala.


### Cada Effect representa um processo de sincronização diferente {/*each-effect-represents-a-separate-synchronization-process*/}

Resista à tentação de adicionar lógica não-relacionada ao seu Effect só porque ela precisa ser executada ao mesmo tempo que o Effect que você escreveu. Por exemplo, suponha que você deseje enviar um evento de analytics quando o usuário visita a sala. Você já tem um Effect dependente de `roomId`, então você pode se sentir tentado a adicionar à chamada aos analytics lá:
...
Mas imagine que, mais tarde, você adicione outra dependência a esse Effect que precise re-estabelecer a conexão. Se esse Effect se ressincronizar, ele também invocará `logVisit(roomId)` para a mesma sala, o que não era sua intenção. Registrar a visita **é um processo separado** da realização de conexão. Escreva-os como dois Effects separados:
...
**Cada Effect no seu código deveria representar um processo de sincronização separado e independente.**

No exemplo acima, deletar um Effect não quebraria a lógica do outro Effect. Esse é um bom indício de que eles sincronizam coisas diferentes, portanto fez sentido separá-los. Por outro lado, se você separar uma peça coesa de lógica em diferentes Effects, o código poderá parecer "mais limpo" mas será [mais díficil de se manter.](/learn/you-might-not-need-an-effect#chains-of-computations) É por isso que você deveria pensar se os processos são o mesmo ou separados, e não se o código parece mais limpo.
...
Nessa versão, o componente `App` passa uma prop booleana ao invés de uma função. Dentro do Effect, você decide qual função utilizar. Como tanto `createEncryptedConnection` quanto `createUnencryptedConnection` estão declaradas fora do componente, elas não são reativas, por isso não precisam ser dependências. Você aprenderá mais sobre isso em [Removendo Dependências de Effects.](/learn/removing-effect-dependencies)

</Solution>

#### Popule uma série de caixas de seleção {/*populate-a-chain-of-select-boxes*/}

Neste exemplo, há duas caixas de seleção. Uma delas permite ao usuário escolher um planeta. A outra o permite escolher um lugar _naquele planeta_. A segunda caixa ainda não funciona. Sua tarefa é fazê-la exibir os lugares do planeta selecionado.

Perceba como a primeira caixa de seleção funciona. Ela popula o state `planetList` com o resultado da chamada à API `"/planets"`. O ID do planeta atualmente selecionado é mantido na variavel de estado `planetId`. Você precisará descobrir aonde adicionar um trecho de código que permita com que a variável de estado `placeList` seja populada com o resultado da chamada à API `"/planets/" + planetId + "/places"`.

Se você fizer essa implementação corretamente, solecionar um planeta deverá popular a lista de lugares. Trocar de planeta deverá mudar a lista de lugares.

<Hint>

Caso você tenha dois processos de sincronização independentes, você precisa escrever dois Effects separados.
...
- A primeira caixa de seleção é sincronizada com a lista remota de planetas remota.
- A segunda caixa de seleção é sincronizada com a lista remota de planetas considerando o `planetId` atual.

É por isso que faz sentido descrever eles como dois Effects separados. Aqui está um exemplo de como você poderia fazer isso:
...
export default function Page() {
  ...
  useEffect(() => {
    if (planetId === '') {
      // Nada é selecionado na primeira caixa até então
      return;
    }
...
Esse código é um pouco repetitivo. Entretanto, essa não é uma boa razão para combiná-lo em um único Effect! Se você fizesse isso, você teria que combinar ambas as listas de dependência em uma única lista, e então mudar o planeta iria requisitar novamente toda a lista de planetas. Effects não são uma ferramenta para reutilização de código. 

Ao invés disso, para reduzir a duplicidade, você pode extrair parte da lógica para um Hook customizado, tal qual o `useSelectOptions` abaixo:
...
Dê uma olhada na aba `useSelectOptions.js` no sandbox para ver como isso funciona. Idealmente, a maior parte dos Effects na sua aplicação deverá eventualmente ser trocada por Hooks customizados, sejam eles escritos por você ou pela comunidade. Hooks customizados escondem a lógica de sincronização, então o componente que os invocam não tomam conhecimento do Effect. Conforme você for trabalhando na sua aplicação, você desenvolverá uma paleta de Hooks que poderá escolher usar, e eventualmente não precisará escrever EFfects tão frequentemente.
